...
{
    RDK_LOG(RDK_LOG_INFO, LOG_UPLOADRRDLOGS, "%s: Entry\n", __FUNCTION__);
    // 1. Parse and validate command-line arguments (UPLOADDIR, ISSUETYPE)
    if (argc != 3) {
        RDK_LOG(RDK_LOG_ERROR, LOG_UPLOADRRDLOGS, "%s: Usage: %s UPLOADDIR ISSUETYPE\n", __FUNCTION__, argv[0]);
    ...
    RDK_LOG(RDK_LOG_INFO, LOG_UPLOADRRDLOGS, "%s: Logging ready\n", __FUNCTION__);

    // 3. Load configuration via Configuration Manager
    rrd_config_t config;
    if (rrd_config_load(&config) != 0) {
        RDK_LOG(RDK_LOG_ERROR, LOG_UPLOADRRDLOGS, "%s: Failed to load configuration.\n", __FUNCTION__);
        fprintf(stderr, "Failed to load configuration.\n");
        return 3;
    }
    RDK_LOG(RDK_LOG_INFO, LOG_UPLOADRRDLOGS, "%s: Configuration loaded\n", __FUNCTION__);
    ...
    if (rrd_sysinfo_get_timestamp(timestamp, sizeof(timestamp)) != 0) {
        RDK_LOG(RDK_LOG_ERROR, LOG_UPLOADRRDLOGS, "%s: Failed to get timestamp.\n", __FUNCTION__);
        fprintf(stderr, "Failed to get timestamp.\n");
        return 5;
    }
    RDK_LOG(RDK_LOG_INFO, LOG_UPLOADRRDLOGS, "%s: MAC: %s, Timestamp: %s\n", __FUNCTION__, mac_addr, timestamp);

    // 5. Validate and prepare log directory
    if (rrd_logproc_validate_source(upload_dir) != 0) {
        RDK_LOG(RDK_LOG_ERROR, LOG_UPLOADRRDLOGS, "%s: Invalid or empty upload directory: %s\n", __FUNCTION__, upload_dir);
        fprintf(stderr, "Invalid or empty upload directory: %s\n", upload_dir);
        return 6;
    }
    if (rrd_logproc_prepare_logs(upload_dir, issue_type) != 0) {
        RDK_LOG(RDK_LOG_ERROR, LOG_UPLOADRRDLOGS, "%s: Failed to prepare logs in %s\n", __FUNCTION__, upload_dir);
        fprintf(stderr, "Failed to prepare logs in %s\n", upload_dir);
        return 7;
    }
    RDK_LOG(RDK_LOG_INFO, LOG_UPLOADRRDLOGS, "%s: Log directory validated and prepared\n", __FUNCTION__);
    ...
    if (rrd_logproc_convert_issue_type(issue_type, issue_type_sanitized, sizeof(issue_type_sanitized)) != 0) {
        RDK_LOG(RDK_LOG_ERROR, LOG_UPLOADRRDLOGS, "%s: Failed to sanitize issue type\n", __FUNCTION__);
        fprintf(stderr, "Failed to sanitize issue type\n");
        return 8;
    }
    RDK_LOG(RDK_LOG_INFO, LOG_UPLOADRRDLOGS, "%s: Issue type sanitized: %s\n", __FUNCTION__, issue_type_sanitized);
    ...
    if (rrd_archive_generate_filename(mac_addr, issue_type_sanitized, timestamp, archive_filename, sizeof(archive_filename)) != 0) {
        RDK_LOG(RDK_LOG_ERROR, LOG_UPLOADRRDLOGS, "%s: Failed to generate archive filename\n", __FUNCTION__);
        fprintf(stderr, "Failed to generate archive filename\n");
        return 9;
    }
    RDK_LOG(RDK_LOG_INFO, LOG_UPLOADRRDLOGS, "%s: Archive filename: %s\n", __FUNCTION__, archive_filename);

    // 8. Create archive
    if (rrd_archive_create(upload_dir, NULL, archive_filename) != 0) {
        RDK_LOG(RDK_LOG_ERROR, LOG_UPLOADRRDLOGS, "%s: Failed to create archive %s\n", __FUNCTION__, archive_filename);
        fprintf(stderr, "Failed to create archive %s\n", archive_filename);
        return 10;
    }
    RDK_LOG(RDK_LOG_INFO, LOG_UPLOADRRDLOGS, "%s: Archive created: %s\n", __FUNCTION__, archive_filename);

    // 9. Upload archive
    if (rrd_upload_execute(config.log_server, config.upload_protocol, config.http_upload_link, upload_dir, archive_filename) != 0) {
        RDK_LOG(RDK_LOG_ERROR, LOG_UPLOADRRDLOGS, "%s: Failed to upload archive\n", __FUNCTION__);
        fprintf(stderr, "Failed to upload archive\n");
        rrd_archive_cleanup(archive_filename);
        return 11;
    }
    RDK_LOG(RDK_LOG_INFO, LOG_UPLOADRRDLOGS, "%s: Archive uploaded successfully\n", __FUNCTION__);

    // 10. Cleanup
    rrd_archive_cleanup(archive_filename);
    rrd_upload_cleanup();
    RDK_LOG(RDK_LOG_INFO, LOG_UPLOADRRDLOGS, "%s: Cleanup complete\n", __FUNCTION__);
