...
int rrd_upload_orchestrate(const char *upload_dir, const char *issue_type)
{
    ...
    RDK_LOG(RDK_LOG_DEBUG, "LOG.RDK.OTEL", "%s: [OTEL] Started child span for rrd_upload_orchestrate\n", __FUNCTION__);
#endif
...
#ifdef ENABLE_OTEL
    RDK_LOG(RDK_LOG_DEBUG, "LOG.RDK.OTEL", "%s: [OTEL] Stopping child span for rrd_upload_orchestrate\n", __FUNCTION__);
