...
Feature: Remote Debugger Static Profile Report With Suffix
  ...
  Scenario: Send RFC event with valid suffixed issue type
    Given the remote debugger is running
    When I trigger the event "Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.RDKRemoteDebugger.IssueType" with value "Device.Info_ab1bghjh"
    ...
    And the remotedebugger request payload should include "MSG=Device.Info_ab1bghjh"

  Scenario: Validate static-profile command execution and upload flow for suffixed issue type
    Given the remotedebugger received the suffixed issue type
    ...
    And remotedebugger should generate upload archive filename for the suffixed issue type
    And remotedebugger should invoke upload with the generated archive

  Scenario: Verify upload result for suffixed issue type report
