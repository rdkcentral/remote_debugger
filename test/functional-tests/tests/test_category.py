...
def test_rdm_rrd_dwnld_status():
    PACKAGE_FOUND = "Package found in Cache...Test"
    ...
    SUCCESS_MSG_SEND = "Message sending Done, ID=0 MSG=Test Size=4 Type=1 AppendMode=0!"
    assert SUCCESS_MSG_SEND in grep_rrdlogs(SUCCESS_MSG_SEND)

    SUCCESS_MSG_RECEIVE = "Message Reception Done for ID=0 MSG=Test TYPE=1..."
    assert SUCCESS_MSG_RECEIVE in grep_rrdlogs(SUCCESS_MSG_RECEIVE)

    ISSUE_CMD_INFO = "Getting Issue command Information for : Test"
    ...
    RUN_DEBUG_CMD = "Run Debug Commands for all issue types in  Test"
